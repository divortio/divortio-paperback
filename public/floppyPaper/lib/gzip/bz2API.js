...
import { Gzip, inflate } from '../../vendor/pako/dist/pako.esm.js';
...
export const BZ_SEQUENCE_ERROR = -1; // Added missing error code
// Note: BZ_MEM_ERROR is typically -4, but we map Pako's error to a generic negative code if needed.
 ...
 */
export function BZ2_bzCompressInit(strm, blockSize100k) {
    if (!strm) return -2; // BZ_PARAM_ERROR

    try {
        // Note: C BZ2_bzCompressInit uses 4 arguments. We only use the level here.
        strm._pako = new Gzip({ level: blockSize100k });
...
export function BZ2_bzCompress(strm, action) {
    ...
    if (action === BZ_FINISH) {
        // Note: The second argument 'true' flushes and ends the pako stream.
    ...
    if (pakoError) {
        // Returning a non-specific negative value as there is no direct Pako -> BZ error code map.
        return -5;
    }
